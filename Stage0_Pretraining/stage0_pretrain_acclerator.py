...
def train_stage0():
    ...
    scheduler = accelerator.prepare(scheduler)  # 单独准备调度器
    # 损失函数
    ...
    for epoch in range(config.max_epochs):
        ...
        for batch_idx, (images, texts) in progress:
            ...
            batch_sizes = accelerator.gather(torch.tensor(len(images), device=accelerator.device))
            batch_size = batch_sizes.sum().item()
        ...
        with torch.no_grad():
            ...
            for images, texts in dataloader:
                ...
                logits_per_image = accelerator.gather(logits_per_image)
                batch_sizes = accelerator.gather(torch.tensor(images.size(0), device=accelerator.device))
                batch_size = batch_sizes.sum().item()
        ...
        if accelerator.is_main_process:
            ...
            # 打印epoch总结
            print(f"\nEpoch {epoch} Summary:")
            print(f"Avg Loss: {epoch_loss/total_samples:.4f} | Top1: {top1_acc:.2f}% | Top5: {top5_acc:.2f}%")

            # 保存checkpoint
            if (epoch + 1) % config.save_interval == 0:
                save_checkpoint(accelerator.unwrap_model(model), optimizer, epoch, config)
