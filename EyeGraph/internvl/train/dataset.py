...
from internvl.serve.vision_helper import RandomAugment
import transformers
...
def build_transform(is_train, task_type, input_size, pad2square=False, normalize_type='imagenet'):
    ...
    if is_train:  # use data augumentation
        if task_type == 'vg':
            T_list = [
                # A.RandomSizedBBoxSafeCrop(height=size, width=size),
                A.RandomResizedCrop(height=input_size, width=input_size, scale=(0.8, 1.0), ratio=(0.9, 1.11)),
                A.RandomRotate90(),
                A.HorizontalFlip(),
                A.VerticalFlip(),
                ...
                A.OneOf([
                        A.Blur(blur_limit=3),
                        A.MedianBlur(blur_limit=3, always_apply=False),
                ], p=0.4),
                A.OneOf([
            ...
            transform = A.Compose(T_list, bbox_params=A.BboxParams(format='albumentations', label_fields=['category_ids']))
        elif task_type == 'vqa_attribute':
            transform = T.Compose([
                T.RandomResizedCrop(input_size, scale=(0.8, 1.0), ratio=(3. / 4., 4. / 3.),
                                    interpolation=InterpolationMode.BICUBIC),
                RandomAugment(3, 5, isPIL=True, augs=['Identity', 'AutoContrast', 'Equalize', 'Brightness',
...
def process_meta_llama_3(
    ...
    sources[0][0]['value'] =  sources[0][0]['value'].replace('<image>', image_tokens, num_image)
    input_ids, attention_mask, labels = [], [], []
    instruction = tokenizer(f"<|start_header_id|>user<|end_header_id|>\n\n{ sources[0][0]['value']}<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n")    
    response = tokenizer(f"{ sources[0][1]['value']}<|eot_id|>")
...
def preprocess(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, turn in enumerate(turns):
            ...
            # print(f'[label  {i}]', tokenizer.decode(target[cur_len + instruction_len: cur_len + turn_len]))
...
def preprocess_phi3(
    ...
    # Mask targets. Only compute loss on the assistant outputs.
    sep = conv.sep + conv.roles[1]  # <|end|><|assistant|>\n
    for conversation, target in zip(conversations, targets):
