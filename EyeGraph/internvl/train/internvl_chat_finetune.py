...
from internvl.train.dataset import (ConcatDataset, TCSLoader,
                                    ...
                                    preprocess_phi3, process_meta_llama_3)
from internvl.train.trainer_monkey_patch import replace_create_optimizer
from PIL import Image, ImageFile, PngImagePlugin
from torch.utils.data import Dataset
from transformers import (EarlyStoppingCallback, AutoConfig, AutoModelForCausalLM, AutoTokenizer,
                          HfArgumentParser, Trainer, TrainingArguments, AutoModel,
                          set_seed)
...
class DataTrainingArguments:
    ...
    eval_path: Optional[str] = field(
        default=None,
        metadata={'help': 'The path of the eval file of datasets.'},
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, template_name, meta, tokenizer, tcs_loader, num_image_token, task_type='norm',
    ...
    def multi_modal_get_item(self, data_item):
        ...
        transform = build_transform(is_train=self.is_train, task_type=self.task_type, input_size=self.image_size,
        ...
        pixel_values = list()
        for image in images:
            if self.task_type == 'vg' and self.is_train:
                image = np.array(image)
                matches = re.findall('\[\[.*?\]\]', data_item['conversations'][1]['value'])
                bboxes = ast.literal_eval(matches[0])
                category_ids = [0]
                transformed = transform(image=image, bboxes=bboxes, category_ids=category_ids)
                trans_bboxes = np.array(transformed['bboxes'], dtype=np.float32).tolist()
                trans_ratios = list()
                for trans_bbox in trans_bboxes:
                    trans_ratios.append([round(trans_bbox[i], 4) for i in range(len(trans_bbox))])
                trans_ratios = str(trans_ratios) 
                data_item['conversations'][1]['value'] = re.sub('\[\[.*?\]\]', trans_ratios, data_item['conversations'][1]['value'])
                # print(data_item['conversations'][1]['value'])
                pixel_value = transformed['image']
            else:
        ...
        elif self.template_name == 'meta-llama3':
            preprocess_function = process_meta_llama_3
        else:
...
def language2instance(text):
    # bbox_re = r'(.*?)\[\[(.*?)(?:\]\]|$)'
    bbox_re = r'^([a-zA-Z]+)\[\[((\d+\.\d{1,}),\s*(\d+\.\d{1,}),\s*(\d+\.\d{1,}),\s*(\d+\.\d{1,}))\]\]$'
    match = re.search(bbox_re, text)
    if match:
        vg_type = match.group(1).strip()
        vg_bbox_str = match.group(2)
        vg_bbox = list(map(float, vg_bbox_str.split(',')))
        return vg_type, vg_bbox
    else:
        return None, None
...
def compute_metrics(pred, tokenizer):
    ...
    end_indices = mask.shape[1] - np.argmax(mask[:, ::-1], axis=1)
    valid_labels = [labels[i, start_indices[i]:end_indices[i]] for i in range(labels.shape[0])]
    valid_preds = [preds[i, start_indices[i]:end_indices[i]] for i in range(preds.shape[0])]
    ...
    decoded_labels = [l.split('<|end|>')[0].strip() for l in decoded_labels]
    decoded_preds = [p.split('<|end|>')[0].strip() for p in decoded_preds]
    
    ious = []
    for pred_text, label_text in zip(decoded_preds, decoded_labels):
        pred_type, pred_bbox = language2instance(pred_text)
        label_type, label_bbox = language2instance(label_text)
        if pred_bbox is not  None and label_bbox is not None:
            iou = calculate_iou(pred_bbox, label_bbox)
            ious.append(iou)
        else:
            return {"miou": -math.inf}
    valid_ious = [iou for iou in ious if iou is not None]
    mean_iou = np.mean(valid_ious)
    print(f'this time form is right and with miou {mean_iou}')
    return {"miou": mean_iou}

def build_datasets(data_path, data_args, tokenizer, tcs_loader, model, group_by_length=False,
                   dynamic_image_size=False, use_thumbnail=False, min_dynamic_patch=1,
    ...
    for ds_name in ds_collections.keys():
        ...
        try:
            dataset = LazySupervisedDataset(
                ...
                task_type=ds_collections[ds_name]['task'],
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        ...
        model = AutoModel.from_pretrained(model_args.vision_path, torch_dtype=torch.bfloat16, trust_remote_code=True)
        # mlp1_ckpt = model.mlp1.state_dict()
        ...
        # llm.eval().to(0)
        # prompt = "Do you recognize fundus pictures?"
        # inputs = tokenizer(prompt, return_tensors="pt").to(0)
        # # Generate
        # generate_ids = llm.generate(inputs.input_ids, max_length=512)
        ...
        model = InternVLChatModel(internvl_chat_config, model.vision_model, llm, tokenizer)
        # model.mlp1.load_state_dict(mlp1_ckpt, strict=True) 
    ...
    logger.info(model.language_model)
    ...
    train_dataset = build_datasets(
        data_args.meta_path, data_args, tokenizer, tcs_loader, model, group_by_length=training_args.group_by_length,
        dynamic_image_size=data_args.dynamic_image_size, use_thumbnail=data_args.use_thumbnail,
        min_dynamic_patch=data_args.min_dynamic_patch, max_dynamic_patch=data_args.max_dynamic_patch,
        normalize_type=data_args.normalize_type)
    
    # eval_dataset = build_datasets(
    #     data_args.eval_path, data_args, tokenizer, tcs_loader, model, group_by_length=training_args.group_by_length,
    #     dynamic_image_size=data_args.dynamic_image_size, use_thumbnail=data_args.use_thumbnail,
    #     min_dynamic_patch=data_args.min_dynamic_patch, max_dynamic_patch=data_args.max_dynamic_patch,
    #     normalize_type=data_args.normalize_type)
    ...
    #     model.language_model.model.embed_tokens.requires_grad = True
    #     model.language_model.lm_head.requires_grad = True
        # model.language_model.model.embed_tokens.weight[:-num_new_tokens].requires_grad = False
        # model.language_model.model.embed_tokens.weight[-num_new_tokens:].requires_grad = True
        # model.language_model.lm_head.weight[:-num_new_tokens].requires_grad = False
        # model.language_model.lm_head.weight[-num_new_tokens:].requires_grad = True
    ...
    if model_args.unfreeze_lm_head:
        model.language_model.base_model.model.model.embed_tokens.weight.requires_grad = True
        model.language_model.base_model.model.lm_head.weight.requires_grad = True
    ...
    data_collator = partial(concat_pad_data_collator, pad_id=tokenizer.pad_token_id)
